...
export default function PengantarSkck() {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [suratskck, setSuratskck] = useState([]);
  ...
  const fetchSuratskck = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(
        "http://147.93.111.133:4000/api/all-skck"
      );

      if (response.data.success) {
        const data = response.data.skckData || []; // Updated to match API response
  ...
  const handlePrint = (surat) => {
    ...
    const printUrl = `${baseUrl}/Main/page/Skck/print?noSurat=${surat.nomor_surat}`;
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await axios.delete(
        "http://147.93.111.133:4000/api/delete-skck",
        {
          data: {
  ...
  const filteredSurat = suratskck.filter(
    (surat) =>
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 text-gray-600">
        <h2 className="text-2xl font-bold mb-6">Surat Pengantar SKCK</h2>
