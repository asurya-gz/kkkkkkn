...
const PrintSurat = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-skck",
          {
            nomor_surat: noSurat,
          }
        );
        setSuratData(response.data.skckData);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-skck",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.setAttribute("download", `skck_${suratData.nama}.docx`);
