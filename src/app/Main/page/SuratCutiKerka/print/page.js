...
const PrintSurat = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-suratcuti",
          {
            no_surat: noSurat,
          }
        );
        setSuratData(response.data.suratCuti);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-surat",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.setAttribute("download", `CutiKerja_${suratData.nama}.docx`);
