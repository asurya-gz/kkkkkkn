...
const SuratContent = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-belummenikah",
          {
            nomor_surat: noSurat,
          }
        );
        setSuratData(response.data.keteranganBelumMenikah);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-belummenikah",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.setAttribute("download", `belummenikah_${suratData.nama}.docx`);
