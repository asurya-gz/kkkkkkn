...
export default function SuratBatasTanah() {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [suratBatasTanah, setSuratBatasTanah] = useState([]);
  ...
  const generateNextNoSurat = (lastNoSurat = "") => {
    try {
      ...
      if (!lastNoSurat) {
        return `590/001/${currentRoman}/${currentYear}`;
      }
  ...
  const fetchSuratBatasTanah = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(
        "http://147.93.111.133:4000/api/all-batastanah"
      );

      if (response.data.success) {
        const data = response.data.batastanah || [];
        ...
        if (data.length > 0) {
          ...
          setSuratBatasTanah(sortedSurat);
  ...
  const handlePrint = (surat) => {
    ...
    const printUrl = `${baseUrl}/Main/page/SuratBatasTanah/print?noSurat=${surat.nomor_surat}`;
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await axios.delete(
        "http://147.93.111.133:4000/api/delete-batastanah",
        {
          data: {
  ...
  const filteredSurat = suratBatasTanah.filter(
    (surat) =>
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 text-gray-600">
        <h2 className="text-2xl font-bold mb-6">Surat Batas Tanah</h2>
