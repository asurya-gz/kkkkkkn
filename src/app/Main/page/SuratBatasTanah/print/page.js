...
const PrintSurat = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-batastanah",
          {
            nomor_surat: noSurat,
          }
        );
        setSuratData(response.data.batastanah);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-batastanah",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.setAttribute("download", `BatasTanah_${suratData.nama}.docx`);
