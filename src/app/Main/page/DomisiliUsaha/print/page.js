...
const SuratContent = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-domisiliusaha",
          {
            nomor_surat: noSurat,
          }
        );
        setSuratData(response.data.domisiliusaha);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-domisiliusaha",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.setAttribute(
        "download",
        `domisiliusaha_${suratData.nama_perusahaan}.docx`
      );
