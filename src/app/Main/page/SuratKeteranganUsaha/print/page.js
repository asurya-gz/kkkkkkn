...
const PrintSurat = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "http://147.93.111.133:4000/api/getno-keteranganusaha",
          {
            nomor_surat: noSurat,
          }
        );
        setSuratData(response.data.keteranganUsaha);
        setLoading(false);
      } catch (err) {
  ...
  const generateSurat = async () => {
    try {
      const response = await axios.post(
        "http://147.93.111.133:4000/api/generate-keteranganusaha",
        suratData,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.setAttribute("download", `KeteranganUsaha_${suratData.nama}.docx`);
